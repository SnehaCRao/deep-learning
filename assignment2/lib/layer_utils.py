...
class LSTM(object):
	...
	def step_forward(self, x, prev_h, prev_c):
		...
		meta = i, f, o, g, a, ai, af, ao, ag, self.params[self.wx_name], self.params[self.wh_name], self.params[self.b_name], prev_h, prev_c, x, next_c, next_h
	...
	def step_backward(self, dnext_h, dnext_c, meta):
		...
		#############################################################################
		i, f, o, g, a, ai, af, ao, ag, Wx, Wh, b, prev_h, prev_c, x, next_c, next_h = meta
	...
	def forward(self, x, h0):
		...
		for t in range(T):
			if t > 0:
				prev_h = h[t - 1]
				prev_c = next_c
			h[t], next_c, meta_next = self.step_forward(
				x[t], prev_h, prev_c)
			self.meta.append(meta_next)
		h = h.transpose(1, 0, 2)
		return h
		##############################################################################
		#							   END OF YOUR CODE							 	 #
	...
	def backward(self, dh):
		...
		for t in reversed(range(T)):
			...
			dc_current = dc_prev
			dx_t, dh_prev, dc_prev, dWx_t, dWh_t, db_t = self.step_backward(
				dh_current, dc_current, self.meta[t])
			dx[t] += dx_t
			dh0 = dh_prev
			dWx += dWx_t
			dWh += dWh_t
