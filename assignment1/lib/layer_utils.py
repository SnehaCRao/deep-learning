...
class fc(object):
    ...
    def forward(self, feat):
        ...
        N = feat.shape[0]
        # print("N ",N)
        D = np.prod(feat.shape[1:])
        feat_reshape = np.reshape(feat, (N, D))
        # print("print feat reshape ",feat_reshape.shape)
        # print("print w shape ",self.params[self.w_name].shape)
        # print("print b shape ",self.params[self.b_name].shape)
        # print(self.params[self.b_name])
    ...
    def backward(self, dprev):
        ...
        dfeat = np.dot(dprev, self.params[self.w_name].T).reshape(feat.shape)  # N x D
        self.grads[self.w_name] = np.dot(feat_reshape.T, dprev)  # D x M
        self.grads[self.b_name] = np.dot(dprev.T, np.ones(N))  # M x 1
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class cross_entropy(object):
    def __init__(self, dim_average=True):
        ...
        #print("in here",self.dim_average)
        self.dLoss = None
        self.label = None
    ...
    def forward(self, feat, label):
        ...
        #############################################################################
        corect_logprobs = np.log(scores[np.arange(N), label])
        if self.dim_average:
