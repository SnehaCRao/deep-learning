...
def train_net(data, model, loss_func, optimizer, batch_size, max_epochs,
	...
	for epoch in range(max_epochs):
		...
		for iter in range(iter_start, iter_end):
			...
			#############################################################################
			loss = None
			ouput = model.forward(data_batch)
			loss = loss_func.forward(ouput, labels_batch)
		...
		"""if N > 1000:
			train_m = np.random.choice(N, 1000)
			data_train_sub = data_train[train_m]
			labels_train_sub = labels_train[train_m]
		else:
			data_train_sub = data_train
			labels_train_sub = labels_train"""
		"""N = data_train.shape[0]
		batch_index = np.random.choice(N, batch_size)
		data_train_sub = data_train[batch_index]
		labels_train_sub = labels_train[batch_index]
		train_output = model.forward(data_train_sub)
		scores_train = softmax(train_output)
		label_pred_train = np.argmax(scores_train, axis=1)
		train_acc = np.mean(label_pred_train == labels_train_sub)
		print("train acc",train_acc)

		# evaluation validation accuracy
		val_output = model.forward(data_val)
		scores_val = softmax(val_output)
		label_pred_val = np.argmax(scores_val, axis=1)
		val_acc = np.mean(label_pred_val == labels_val)
